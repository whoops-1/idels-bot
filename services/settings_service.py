...
async def get_chat_settings(chat_id: int) -> ChatSettings:
    db = await get_db()
    rows = await db.fetch("SELECT * FROM chats WHERE chat_id = $1", chat_id)
    if not rows:
        await db.execute("INSERT INTO chats (chat_id) VALUES ($1)", chat_id)
        rows = await db.fetch("SELECT * FROM chats WHERE chat_id = $1", chat_id)
...
async def update_chat_setting(chat_id: int, key: str, value) -> None:
    ...
    db = await get_db()
    await db.execute(
        f"UPDATE chats SET {key} = $1, updated_at = $2 WHERE chat_id = $3",
...
async def update_link_allowlist(chat_id: int, allowlist: list[str]) -> None:
    db = await get_db()
    await db.execute(
        "UPDATE chats SET link_allowlist = $1, updated_at = $2 WHERE chat_id = $3",
...
async def get_chat_owner_id(chat_id: int) -> int:
    db = await get_db()
    rows = await db.fetch("SELECT owner_id FROM chats WHERE chat_id = $1", chat_id)
    return rows[0]["owner_id"] if rows else 0
...
async def log_action(chat_id: int, action_type: str, actor_id: int, target_id: int = 0, details: str = "") -> None:
    db = await get_db()
    await db.execute(
        "INSERT INTO action_log (chat_id, action_type, actor_id, target_id, details) VALUES ($1, $2, $3, $4, $5)",
...
async def get_trigger(chat_id: int, keyword: str) -> dict | None:
    db = await get_db()
    rows = await db.fetch(
        "SELECT * FROM triggers WHERE chat_id = $1 AND keyword = $2",
...
async def get_all_triggers(chat_id: int) -> list[dict]:
    db = await get_db()
    rows = await db.fetch(
        "SELECT * FROM triggers WHERE chat_id = $1 ORDER BY keyword", chat_id,
    )
...
async def add_trigger(chat_id: int, keyword: str, response: str, created_by: int, is_regex: int = 0, delete_trigger: int = 0) -> None:
    ...
    await db.execute(
        ...
        "VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT(chat_id, keyword) DO UPDATE SET response = EXCLUDED.response",
...
async def remove_trigger(chat_id: int, keyword: str) -> bool:
    ...
    result = await db.execute(
        "DELETE FROM triggers WHERE chat_id = $1 AND keyword = $2",
        chat_id, keyword.lower(),
    )
...
async def add_user_note(chat_id: int, user_id: int, note: str, created_by: int) -> int:
    ...
    row = await db.fetchrow(
        "INSERT INTO user_notes (chat_id, user_id, note, created_by) VALUES ($1, $2, $3, $4) RETURNING id",
...
async def get_user_notes(chat_id: int, user_id: int) -> list[dict]:
    db = await get_db()
    rows = await db.fetch(
        "SELECT * FROM user_notes WHERE chat_id = $1 AND user_id = $2 ORDER BY created_at DESC",
        chat_id, user_id,
    )
    return [dict(r) for r in rows]
...
async def delete_user_note(note_id: int, chat_id: int) -> bool:
    db = await get_db()
    result = await db.execute(
        "DELETE FROM user_notes WHERE id = $1 AND chat_id = $2", note_id, chat_id,
    )
    return "DELETE 1" in str(result)
...
async def create_federation(name: str, owner_id: int, description: str = "") -> int:
    db = await get_db()
    row = await db.fetchrow(
        "INSERT INTO federations (name, owner_id, description) VALUES ($1, $2, $3) RETURNING id",
...
async def join_federation(federation_id: int, chat_id: int) -> None:
    db = await get_db()
    await db.execute(
        "INSERT INTO federation_members (federation_id, chat_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
        federation_id, chat_id,
    )
    await db.execute(
        "UPDATE chats SET federation_id = $1 WHERE chat_id = $2",
        federation_id, chat_id,
    )
...
async def leave_federation(chat_id: int) -> None:
    ...
    await db.execute("UPDATE chats SET federation_id = 0 WHERE chat_id = $1", chat_id)


async def get_federation_by_name(name: str) -> dict | None:
    db = await get_db()
    rows = await db.fetch("SELECT * FROM federations WHERE name = $1", name)
    return dict(rows[0]) if rows else None


async def get_federation_by_id(fed_id: int) -> dict | None:
    db = await get_db()
    rows = await db.fetch("SELECT * FROM federations WHERE id = $1", fed_id)
    return dict(rows[0]) if rows else None
...
async def fed_ban_user(federation_id: int, user_id: int, reason: str, banned_by: int) -> None:
    ...
    await db.execute(
        ...
        "ON CONFLICT(federation_id, user_id) DO UPDATE SET reason = EXCLUDED.reason, banned_by = EXCLUDED.banned_by",
...
async def is_fed_banned(federation_id: int, user_id: int) -> dict | None:
    ...
    db = await get_db()
    rows = await db.fetch(
        "SELECT * FROM federation_bans WHERE federation_id = $1 AND user_id = $2",
        federation_id, user_id,
    )
    return dict(rows[0]) if rows else None
...
async def create_captcha_session(chat_id: int, user_id: int, answer: str, timeout: int, message_id: int = 0) -> None:
    ...
    now = int(time.time())
    await db.execute(
        "DELETE FROM captcha_sessions WHERE chat_id = $1 AND user_id = $2 AND solved = 0",
        chat_id, user_id,
    )
    await db.execute(
        "INSERT INTO captcha_sessions (chat_id, user_id, answer, message_id, expires_at) VALUES ($1, $2, $3, $4, $5)",
...
async def verify_captcha(chat_id: int, user_id: int, answer: str) -> bool:
    ...
    now = int(time.time())
    rows = await db.fetch(
        "SELECT * FROM captcha_sessions WHERE chat_id = $1 AND user_id = $2 AND solved = 0 AND expires_at > $3",
        chat_id, user_id, now,
    )
    if not rows:
        return False
    ...
    if session["answer"].lower() == answer.lower().strip():
        await db.execute("UPDATE captcha_sessions SET solved = 1 WHERE id = $1", session["id"])
        return True
    return False
...
async def get_expired_captcha_sessions() -> list[dict]:
    ...
    now = int(time.time())
    rows = await db.fetch(
        "SELECT * FROM captcha_sessions WHERE solved = 0 AND expires_at <= $1",
        now,
    )
    return [dict(r) for r in rows]
